    ...
    class Assignments:
        ...
        def get(
            ...
            if is_float == "z":
                ...
                for item in range(0, len(assignment), n_cols):
                    summary += (
                        " ".join(list(map(str, assignment[item : item + n_cols])))
                        + "\n"
                    )
            if deadline != "-":
                summary += f"\n🕚 Дедлайн: {deadline}\n"
        ...
        def failure(err: str):
            cases = {
                "StudentNotFound": "⛔ Для получения оценок нужно зарегистрироваться."
                + "Команда /start поможет с регистрацией",
            }
            return cases.get(err, "😟 Возникли неполадки")

    class Sub:
        START = "📜✍ Оформляю подписку на уведомления..."
        SUBBED = "👌 Подписка на уведомления оформлена"
        GRADES_CHANGED = (
            "💌 Твои оценки изменились! Вызови команду /grades, чтобы узнать!"
        )

        @staticmethod
        def failure(err: str):
            cases = {
                "StudentNotFound": "⛔ Для подписки нужно зарегистрироваться."
                + "Команда /start поможет с регистрацией",
                "StudentAlreadySubbed": "💁‍♀️ Подписка уже оформлена ранее",
            }
            return cases.get(err, "😟 Возникли неполадки")

    class Unsub:
        START = "📜✍ Оформляю отписку от уведомлений..."
        UNSUBBED = "🙄 Подписка на уведомления отменена"
