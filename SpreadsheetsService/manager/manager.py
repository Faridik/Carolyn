...
class Manager:
    ...
    def get_values(self, range_name: str = 'StudentList') -> list:       # TODO: change subject
    ...
    def get_student_by_name(self, name: str) -> Student:
        ...
        f = lambda row: name == row[STUDENT_NAMES]
        try:
            row = next(filter(f, values))
            return Student(int(row[STUDENT_NUMBERS]), name,
                                row[GROUP_IDS], row[TELEGRAM_IDS])
    ...
    def get_student_by_tg_id(self, tg_id: int) -> Student:
        ...
        try:
            row = next(filter(f, values))
            return Student(int(row[STUDENT_NUMBERS]), row[STUDENT_NAMES],
                                row[GROUP_IDS], tg_id)
        except StopIteration:
    ...
    def set_assignments_for_student(self, student: Student):
        ...
        for row in filter(f,values):
            ...
            student.add_assignment(Assignment(row[ASSIGNMENT_NAMES], ass_values, 
                                                row[ASSIGNMENT_WEIGHTS]))
